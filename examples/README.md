...
| `canary_added` | bool | Whether `/pause` auto-added a canary line |
| `paused_at` | string | ISO-8601 UTC timestamp |
| `label` | string | Optional short label (informational) |

To resume, `/resume` reads this file and `CronCreate`s each entry. To
...
- `started` — when a loop was first scheduled (`/loop` or `/resume`)
- `paused` — when `/pause` saved + cancelled it
- `resumed` — when `/resume` recreated the cron
